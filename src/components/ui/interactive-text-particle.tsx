...
const ParticleTextEffect: React.FC<ParticleTextEffectProps> = ({
  ...
  const dottify = () => {
    ...
    // Optimize pixel scanning
    for (let i = 0; i < data.length; i += 4) {
      const alpha = data[i + 3];
      if (alpha > 0) {
        const x = (i / 4) % textBox.w;
        const y = Math.floor((i / 4) / textBox.w);
  ...
  const write = () => {
    ...
    textBox.str = text;
    // Adjust font size calculation for longer text
    textBox.h = Math.floor(canvas.width / (textBox.str.length * 0.8)); 
